...
function filterVehicles(list: Vehicle[], category: Category): Vehicle[] {
  ...
  if (category === "automatic") {
    return list.filter((v) => v.transmission === "Automatic");
  }
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="text-center">
          ...
          <p className="mx-auto mt-4 max-w-2xl text-base text-gray-500 sm:text-lg">
            A wide range of well-maintained vehicles to suit your needs and
            budget.
          </p>
        </div>
          ...
          {loading ? (
            <div className="flex gap-5 overflow-hidden py-4">
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="h-[380px] w-[280px] shrink-0 animate-pulse rounded-2xl bg-gray-100 sm:w-[300px]"
                />
              ))}
            </div>
          ) : filtered.length === 0 ? (
            ...
            >
              {filtered.map((vehicle) => (
                <article
                  key={vehicle.id}
                  className="w-[280px] shrink-0 rounded-3xl bg-white shadow-md ring-1 ring-gray-100 sm:w-[300px]"
                >
